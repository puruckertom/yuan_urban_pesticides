...
pwcdf_o <- read.csv(paste(pwcdir,"output/output_PGC010_parent_only_Custom_Parent_daily.csv", sep=""), header= FALSE, sep= ",",
...
for(Ite in 1:Nsims){
  ...
  a[4]= paste(pwcdir_output,"17719_grid_roseville.wea")
...
# ---------------------------------------------------------------------------
  ...
  # ---------------------------- PWC Output ---------------------------------
  pwcdf <- read.csv(paste(newdir,"/","output_PGC010_parent_only_Custom_Parent_daily",".csv", sep=""), header= FALSE, sep= ",",
                    skip = 5, stringsAsFactors = FALSE, row.names=NULL)
  ...
  con_almond <- file(paste(newdir,"/","output_PGC010_parent_only_Custom_Parent",".txt", sep=""))
