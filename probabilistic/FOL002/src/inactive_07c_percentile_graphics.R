...
pwc_pplot <- ggplot(percentiles, aes(x=day, group=1)) +
  ...
  theme(axis.text.y = element_text(size = 16))+ #axis text size
  theme(axis.text.x = element_text(size = 16))+
  theme(axis.title.y = element_text(size = 18))+ #y axis label
...
pwc_pplot <- ggplot(percentiles, aes(x=day, group=1)) +
  ...
  scale_x_date(date_breaks="1 year", date_labels="%m-%d-%y", limits=as.Date(c('2011-01-01', '2014-12-31'))) +
  scale_y_continuous(trans="log10", breaks=trans_breaks("log10", function(x) 10^x), 
                     labels=trans_format("log10", math_format(10^.x)), limits=c(NA,9000)) +
