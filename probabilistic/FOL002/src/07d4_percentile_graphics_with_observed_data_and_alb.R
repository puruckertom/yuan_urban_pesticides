...
# plot
png(filename= paste(pwcdir, "figures/percentile_with_observed_alb_09-14_pwc_ave_h2.png", sep=""),width=20, height=10, units="in",res=300) 
...
# plot
png(filename= paste(pwcdir, "figures/percentile_with_observed_alb_09-14_pwc_ave_benthic.png", sep=""),width=20, height=10, units="in",res=300) 
...
pwc_pplot <- ggplot(percentiles, aes(x=day, group=1)) +
  ...
  theme(legend.position = "bottom") +
  scale_fill_manual(name="", values=c("3 SD"=sd3, "2 SD"=sd2, "1 SD" =sd1))+
  scale_color_manual(name="", values=c("Deterministic"=det,"Probabilistic Median" =med, 
                                       "Acute/Chronic Invertebrate"=invert, "Acute/Chronic Fish" = fish))
