...
if(Sys.info()[4]=="LZ2626UECHELSVI"){
  determdir <- "C:/Users/echelsvi/git/yuan_urban_pesticides/dummy/dummy_water2/"
}
print(paste("Root directory location: ", determdir, sep=""))


con_przm5 <- file(paste(determdir,"PRZM5",".inp",sep=""))
a_old=readLines(con_przm5)
a=readLines(con_przm5)
close(con_przm5)
  
# update the files (weather file, PRZM output file)
a[4]= paste(determdir,"17484_grid_folsom.wea",sep="")
a[6]= paste(determdir,"outputs/output.zts",sep="")
  

# create a przm input
newdir <- paste0(determdir,"outputs")
print(newdir)
...
# copy weather file
weather_input <- paste(determdir, "17484_grid_folsom.wea", sep="")
...
con <- file(paste(determdir, "vvwmTransfer",".txt",sep=""))
l_old=readLines(con)
l=readLines(con)
close(con)
